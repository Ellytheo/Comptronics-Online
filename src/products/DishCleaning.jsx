...
const DishCleaning = () => {
  ...
  const getProducts = async () => {
    ...
    setError("");
    try {
      const response = await axios.get("https://0909El.pythonanywhere.com/api/get_dish_cleaning");
  ...
  return (
    <div className="container-fluid">
      ...
      {/* Swiper Carousel for 3D Effect */}
      <div className="w-full max-w-3xl mx-auto mb-4 cylindrical-track-container">
        <Swiper
          ...
          autoplay={{ delay: 3000 }}
          loop
          speed={3000} // Slow speed for smooth rotation effect
          slidesPerView={1} // Display multiple slides at once to create 3D feel
          centeredSlides={true} // Ensure the current slide is always centered
          className="swiper-cylindrical"
        >
          {[pic1, pic8, pic16, pic20, pic27, pic38, pic40].map((pic, idx) => (
            <SwiperSlide key={idx}>
              <div className="cylindrical-card">
                <img
                  src={pic}
                  alt={`Slide ${idx + 1}`}
                  className="cylindrical-card-img"
                />
              </div>
            </SwiperSlide>
          ))}
